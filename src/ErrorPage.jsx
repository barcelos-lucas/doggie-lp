...
export default function ErrorPage({ code }) {
  ...
  return <main className="error-page">
    ...
    <section className="error-card">
      ...
      <div className="error-actions">{missing ? <><a className="button cookie-primary" href="/"><House size={19} aria-hidden="true" />Voltar ao início</a><a className="button whatsapp" href={whatsappUrl()} target="_blank" rel="noopener noreferrer"><WhatsAppMark /><span>Falar com a Doggie</span></a></> : <><button type="button" className="button cookie-primary" onClick={() => window.location.reload()}><ArrowClockwise size={19} aria-hidden="true" />Tentar novamente</button><a className="button whatsapp" href={whatsappUrl()} target="_blank" rel="noopener noreferrer"><WhatsAppMark /><span>Falar com a Tia Bia</span></a><a className="error-home-link" href="/"><House size={16} aria-hidden="true" />Voltar ao início</a></>}</div>
